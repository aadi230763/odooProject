...
import { departmentsApi, employeesApi, type Department, type OrgEmployee } from '../../api/org';
...
function ProgressBar({ cycle }: { cycle: AuditCycle }) {
  ...
  if (total === 0) return <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>No assets</span>;
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      >
        <div style={{ width: `${verifiedPct}%`, background: 'var(--success, #22c55e)' }} />
        <div style={{ width: `${missingPct}%`, background: 'var(--danger, #ef4444)' }} />
        <div style={{ width: `${damagedPct}%`, background: 'var(--warning, #f59e0b)' }} />
      ...
      >
        ...
        <span style={{ color: 'var(--danger, #ef4444)' }}>✕ {cycle.missing_count}</span>
        <span style={{ color: 'var(--warning, #f59e0b)' }}>⚠ {cycle.damaged_count}</span>
...
export function AuditCyclesPage() {
  ...
  const [selectedAuditorIds, setSelectedAuditorIds] = useState<Set<number>>(new Set());
  ...
  const openCreate = async () => {
    ...
    } catch { /* ignore — dept is optional */ }
    setCreateForm({ name: '', start_date: '', end_date: '', scope_department_id: '', scope_location: '' });
    setShowCreate(true);
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.error('Failed to create audit cycle.', err instanceof ApiError ? err.message : undefined);
    } finally {
  ...
  const handleAssign = async () => {
    ...
    try {
      await auditsApi.assignAuditors(selected.id, Array.from(selectedAuditorIds));
  ...
  const openItems = async (cycle: AuditCycle) => {
    ...
    try {
      ...
      res.audit_items.forEach(i => { drafts[i.id] = i.notes ?? ''; });
  ...
  const handleMarkItem = async (item: AuditItem, result: AuditItemResult) => {
    ...
    try {
      ...
      setItems(prev => prev.map(i => i.id === item.id ? res.audit_item : i));
      await refreshSelected(selected.id);
    } catch (err) {
      toast.error('Failed to mark item.', err instanceof ApiError ? err.message : undefined);
    } finally {
      setMarkingItem(null);
    }
  };
  ...
  const handleClose = async (cycle: AuditCycle) => {
    ...
    try {
      ...
      toast.error('Failed to close cycle.', err instanceof ApiError ? err.message : undefined);
    }
  };
  ...
  return (
    <>
      ...
      {/* Filters */}
      <div style={{ display: 'flex', gap: 'var(--sp-3)', marginBottom: 'var(--sp-4)', maxWidth: '220px' }}>
        <Select
      ...
      <div style={{ marginBottom: 'var(--sp-3)', fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
        {loading ? <Spinner size="xs" label="Loading" /> : `${cycles.length} cycle${cycles.length !== 1 ? 's' : ''}`}
      </div>
            ...
            },
            {
              ...
              render: c => (
                <div style={{ fontSize: 'var(--text-sm)' }}>
                  {c.scope_department_name && <div>Dept: {c.scope_department_name}</div>}
                  {c.scope_location && <div>Location: {c.scope_location}</div>}
                  {!c.scope_department_name && !c.scope_location && <span style={{ color: 'var(--text-muted)' }}>All assets</span>}
                </div>
              ),
            },
            {
            ...
            },
            {
              ...
              render: c => (
                <div style={{ display: 'flex', gap: 'var(--sp-2)', flexWrap: 'wrap' }}>
                  <Button variant="ghost" size="sm" onClick={() => openDetail(c)}>Detail</Button>
                  <Button variant="ghost" size="sm" onClick={() => openItems(c)}>Verify</Button>
                  <Button variant="ghost" size="sm" onClick={() => openReport(c)}>Report</Button>
                </div>
              ),
            },
          ]}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="primary" loading={createSaving} onClick={handleCreate}>Create</Button>
          </>
        }
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
          ...
          <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', margin: 0 }}>
            Scope — fill at least one to limit which assets are included. Leave both blank to include all active assets.
          </p>
      ...
      {selected && (
        <Modal
          ...
          title={selected.name}
          footer={
            <div style={{ display: 'flex', gap: 'var(--sp-2)', justifyContent: 'space-between', width: '100%' }}>
              <div style={{ display: 'flex', gap: 'var(--sp-2)' }}>
                {selected.status === 'open' && isManager && (
                  <>
                    <Button variant="secondary" size="sm" onClick={() => { setShowDetail(false); openAssign(selected); }}>
                      Assign Auditors
                    </Button>
                    <Button variant="danger" size="sm" onClick={() => handleClose(selected)}>
                      Close Cycle
                    </Button>
                  </>
                )}
                <Button variant="ghost" size="sm" onClick={() => { setShowDetail(false); openItems(selected); }}>
                  Verify Assets
                </Button>
                <Button variant="ghost" size="sm" onClick={() => { setShowDetail(false); openReport(selected); }}>
                  Discrepancy Report
                </Button>
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-3)' }}>
              <InfoField label="Status">
                <Badge variant={selected.status === 'open' ? 'primary' : 'muted'}>
                  {selected.status === 'open' ? 'Open' : 'Closed'}
                </Badge>
              </InfoField>
              <InfoField label="Date Range" value={`${fmtDate(selected.start_date)} → ${fmtDate(selected.end_date)}`} />
              <InfoField label="Scope — Dept" value={selected.scope_department_name ?? '—'} />
              <InfoField label="Scope — Location" value={selected.scope_location ?? '—'} />
              <InfoField label="Created By" value={selected.creator_name ?? '—'} />
              <InfoField label="Auditors" value={selected.auditor_names.join(', ') || '—'} />
            </div>

            <div>
              <p style={{ fontSize: 'var(--text-xs)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 'var(--sp-2)' }}>
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <Button variant="primary" loading={assignSaving} onClick={handleAssign}>Save</Button>
            </>
          }
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)', maxHeight: '320px', overflowY: 'auto' }}>
            {employees.map(emp => (
              <label
                key={emp.id}
                style={{
                  ...
                  background: selectedAuditorIds.has(emp.id) ? 'var(--primary-subtle, rgba(59,130,246,0.08))' : 'var(--bg-surface-2)',
                  cursor: 'pointer',
              ...
              >
                ...
                />
                <div>
                  <div style={{ fontWeight: 'var(--fw-medium)' }}>{emp.name}</div>
      ...
      {selected && (
        <Modal
          ...
          footer={<Button variant="secondary" onClick={() => setShowItems(false)}>Close</Button>}
        >
          {itemsLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)', maxHeight: '480px', overflowY: 'auto' }}>
              {items.map(item => (
                <div
                  key={item.id}
                  style={{
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <strong
                        style={{ color: 'var(--primary)', cursor: 'pointer', fontSize: 'var(--text-sm)' }}
                        onClick={() => { setShowItems(false); navigate(`/assets/${item.asset_id}`); }}
                      >
                        {item.asset_tag}
                      </strong>
                    ...
                    <Badge variant={RESULT_BADGE[item.result]}>{RESULT_LABEL[item.result]}</Badge>
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: 'var(--sp-2)', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                      ...
                      {(['verified', 'missing', 'damaged'] as AuditItemResult[]).map(r => (
                        <Button
                          ...
                          size="sm"
                          variant={
                            item.result === r ? 'primary'
                            : r === 'verified' ? 'secondary'
                            : r === 'missing' ? 'danger'
                            : 'secondary'
                          }
                        ...
                        >
                          {r === 'verified' ? '✓' : r === 'missing' ? '✕' : '⚠'} {RESULT_LABEL[r]}
                        </Button>
                      ))}
                    </div>
                  )}
      ...
      {selected && (
        <Modal
          ...
          footer={<Button variant="secondary" onClick={() => setShowReport(false)}>Close</Button>}
        >
          {reportLoading ? (
            ...
            <>
              <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', marginBottom: 'var(--sp-3)' }}>
                {reportItems.length} item{reportItems.length !== 1 ? 's' : ''} with discrepancies (Missing or Damaged).
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)', maxHeight: '420px', overflowY: 'auto' }}>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--sp-1)' }}>
                      <div>
                        <strong
                          style={{ color: 'var(--primary)', cursor: 'pointer', fontSize: 'var(--text-sm)' }}
                          onClick={() => { setShowReport(false); navigate(`/assets/${item.asset_id}`); }}
                        >
                          {item.asset_tag}
                        </strong>
                        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginLeft: 'var(--sp-2)' }}>
