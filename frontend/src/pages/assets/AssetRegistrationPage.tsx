...
export function AssetRegistrationPage() {
  ...
  return (
    <>
      ...
      <form className="card" onSubmit={handleSubmit} style={{ maxWidth: '600px', display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
        <Input
          id="asset-name"
          label="Asset Name"
          required
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', marginTop: 'var(--sp-2)' }}>
          ...
          />
          <label htmlFor="asset-bookable" style={{ fontSize: 'var(--text-sm)', color: 'var(--text-primary)' }}>
            This is a shared resource (bookable by time-slot)
          </label>
        </div>

        <div style={{ marginTop: 'var(--sp-2)' }}>
          <label style={{ display: 'block', fontSize: 'var(--text-sm)', fontWeight: 'var(--fw-medium)', marginBottom: 'var(--sp-1)' }}>
            Photo (Optional)
          </label>
          <input
            type="file"
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--sp-3)', marginTop: 'var(--sp-4)' }}>
          ...
          <Button variant="primary" type="submit" loading={saving}>Register Asset</Button>
        </div>
      </form>
    </>
  );
